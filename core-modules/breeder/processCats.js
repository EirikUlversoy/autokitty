async function processCats(catPortion, copyOfCats, catDictionary, GeneDecoder) {
   ...
   _getSortedArrayOfScoredMutaCatsFromDictionary = function(scores){
	...
	_makeMutationScoreDictionarySingularHC = function(cat, copyOfCats, catDictionary){
      ...
      if(cat.generation == 0 || cat_B.generation == 0){
    ...
    var score = 0;
      ...
      for(var part in mutationOrdered){
        partner = catDictionary[mutationOrdered[part][0]];
  			if((isValidMatch(nCat, partner)) && (mutationOrdered[part][1] > 0.0) && (nCat.id != partner.id)){
		...
		if(partner != undefined){
			...
			*/
			...
			if(parseInt(catDictionary[""+partner.id].cooldownIndex,10) <= parseInt(catDictionary[""+nCat.id].cooldownIndex,10)){
			 	internalBreedingPairs.push(new BreedingPair(partner.id, nCat.id, score));
			} else {
				internalBreedingPairs.push(new BreedingPair(nCat.id, partner.id, score));
			}
		}
	}
