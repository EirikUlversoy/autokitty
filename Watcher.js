...
var net = require('net');
var kitty_clock_address = 0x06012c8cf97bead5deae237070f9587f8e7a266d;
...
function fetch(id){
	...
	return Promise.delay(3000, id).then(CKClient.getUserKitties(kitty_clock_address,64,id*20)
...
function getCatsLoop(n_cats){
	console.log(cats.length + " before getcatsloop..");
...
function inspect(calls){
	...
	var newGenes = 0;
	var potentialNGCats = [];
	for(var kitty in cats){
		kitty = cats[kitty];
		var KAISequence = GeneDecoder.translateGenesToKai(kitty.genes);
		var KaiGroups = [];
		for(var x = 0; x<KAISequence.length/4; x++){
			var nextGroup = KAISequence.substring(x*4,(x+1)*4);
			KaiGroups.push(nextGroup);
		}
		var geneArrays = GeneDecoder.probeCattributes(KaiGroups);
		if(geneArrays.includes(true)){
			potentialNGCats.push(kitty.id);
			newGenes++;
		}
	}
	console.log("Done inspecting.");
	console.log("New genes: " + newGenes);
	console.log("Interesting kittens: " + potentialNGCats);

}

loopGetUserKitties().then(inspect);
