...
function getAllKittenNumbers(maxNumber){
...
function separateByGeneration(){
	for (var kitten in cats){
		kitten = cats[kitten];
		if(!genDict[kitten.generation]){
			genDict[kitten.generation] = [];
		}
		genDict[kitten.generation].push(kitten);
	}
	for(x = 0; x < 25; x++){
		outputGeneration(x);
	}
...
function outputGeneration(generation){
	var catIDs = [];
	var theCats = genDict[generation];
...
kittens = getAllKittenNumbers(747525);
//kittens = getAllKittenNumbers(40000);


getOwnershipOfCatsLoop(kittens).then(getCatsLoop).then(separateByGeneration);